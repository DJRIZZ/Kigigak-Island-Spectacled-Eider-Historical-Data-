...
6. Combining the the .dbf files from each year (years 1994-2005)
...
The code chunk above compares the "resight" data files from years 1992-2005 to the reference data frame created in section 4. This follows the same structure and serves the same function as comparing the 'header' and 'markdata' data. 

For the resight data collected from years 1992-2005, the field 'TARSALCODE' is missing from the dbf in all years. After looking at the dbf tables, the data was collected and input under the name: 'TARSUS' and can be changed to match the protocol specifications.  
...
The code chunk above compares the "visit" data files from years 1992-2005 to the reference data frame created in section 4. This follows the same structure and serves the same function as comparing the 'header', 'markdata', and 'resight' data. 

For the visit data collected from 1992-2005, there appears to be no missing column names in the dbf files. There are columns that are categorized in the "missing in reference" field, this indicates that the visit data collected from these years have at least all of the required fields specified by the protocols, and include extra columns that are not necessarily specified by name in the protocols.
...
The code chunk above compares the "egg" data files from years 1992-2005 to the reference data frame created in section 4. This follows the same structure and serves the same function as comparing the 'header', 'markdata','resight', and 'visit' data. 

For the egg data collected from 1992-2005, there appears to be no missing column names in the dbf files. There are columns that are categorized in the "missing in reference" field, this indicates that the egg data collected from these years have at least all of the required fields specified by the protocols, and include extra columns that are not necessarily specified by name in the protocols. After year 2005, there was no more egg data collected. 
...
```
<details><summary>markdata_combined_data1992.2005 TABLE *(Click to expand)*</summary>
```{r}
knitr::kable(markdata_combined_data1992.2005)
```
</details>

The code chunk above, processes the markdata dbf files from years 1992-2005 and combines them into one data frame. This code first creates an empty list in which to store the data frames created from each year, then loops through each of dbf files and reads them from the markdata_dbf_files_eider1992.2005 value created in 5b. I modify the data frame by adding a column: 'YEAR' to ensure that the data remains organized to the year that is was collected in. The year is extracted from the folder name. The markdata files are then combined into one data frame labeled: "markdata_combined_data1992.2005". The DATE data input during the year 2000, was input as year 1900 and will be later changed.
...
```
<details><summary>resight_combined_data1992.2005 TABLE *(Click to expand)*</summary>
```{r}
knitr::kable(resight_combined_data1992.2005)
```
</details>

The code chunk above, processes the resight dbf files from years 1992-2005 and combines them into one data frame. This code first creates an empty list in which to store the data frames created from each year, then loops through each of dbf files and reads them from the resight_dbf_files_eider1992.2005 value created in 5c. I modify the data frame by adding a column: 'Year' to ensure that the data remains organized to the year that is was collected in. The year is extracted from the folder name. The modified data frames were then attempted to combine and an error occurred specifying that the data type in the "NORTHING" column was not consistent across all data frames. Code was then added to convert the data in the "NORTHING" column to a consistent to character/string data form. 
...
```
<details><summary>visit_combined_data1992.2005 TABLE *(Click to expand)*</summary>
```{r}
knitr::kable(visit_combined_data1992.2005)
```
</details>

The code chunk above, processes the visit dbf files from years 1992-2005 and combines them into one data frame. This code first creates an empty list in which to store the data frames created from each year, then loops through each of dbf files and reads them from the visit_dbf_files_eider1992.2005 value created in 5d. I modify the data frame by adding a column: 'Year' to ensure that the data remains organized to the year that it was collected in. The year is extracted from the folder name. The visit data files are then combined into a single data frame labeled: "visit_combined_data_1992.2005".
...
```
<details><summary>egg_combined_data1992.2005 TABLE *(Click to expand)*</summary>
```{r}
knitr::kable(egg_combined_data1992.2005)
```
</details>

The code chunk above, processes the egg dbf files from years 1992-2005 and combines them into one data frame. This code first creates an empty list in which to store the data frames created from each year, then loops through each of dbf files and reads them from the egg_dbf_files_eider1992.2005 value created in 5e. I modify the data frame by adding a column: 'Year' to ensure that the data remains organized to the year that is was collected in. The year is extracted from the folder name. The modified data frames were then attempted to combine and an error occurred specifying that the data type in the "EGGNO" column was not consistent across all data frames. Code was then added to convert the data in the "EGGNO" column to a consistent integer data form. 
...
The code chunk above processes the markdata files from years 2006-2015 and combines them into a single data frame. This code firsts creates an empty list in which to store the data frames that are created from each year as they are read through. For the column names that will need to be changed, an empty variable as well as a predefined list of names that need to be changed and what they need to be changed into. Then the code is broken up into two loops. The first, reads all the excel files found in the list 'markdata_excel_files_eider2006.2015' created in 11b, then creates a new column in each file with the year, extracted from the folder name. Then all of the column names in each data frame are checked, and renamed according the column_rename_map_markdata. The code then standardizes band number information that was recorded in different formats across years and data entry systems. Columns BANDNU_SUFFIX and BANDNU_COMPLETE were created to store band number data evaluated from BAND_RAW and separated into these new columns. If the band cumber is complete, it will go into BANDNU_COMPLETE; if it is just the suffix, it will be paired with the PREFIXNUMB and entered into BANDNU_COMPLETE. Columns including 'RECAP','PLOT', 'CULMEN', 'TARSUS', and 'WT' are changed to have a consistent data type across files. All of the column names are now stored in the variable 'all_columns'; and the files are checked for missing columns, and missing columns are added to the data frames with NAs. The second loop is then initiated and reads through the excel files again, and applies the renaming map again in case any file had other inconsistent names. The column names are then read through and checked to ensure that they match the column names stored in the 'all_columns' variable. Columns including 'EASTING', 'NORTHING', 'DATE', 'RECAP, and 'PLOT' are converted to be standard across files. The updated data frames are added to the 'markdata_combined_list2006.2015' list and used to combine into the single data frame, 'markdata_combined_data2006.2015'. 
During some years, broods were captured and marked. This data is then read and combined with the previously created markdata table. The code first creates a list of all excel files containing "brood" or "broods" in the file name. Harmonize functions are defined to ensure columns with the same name have consistent data types before they are merged with markdata dataframe. For each file, the year is extracted from the folder name and stored in a new 'YEAR' column and all column names are standardized. In some years, the excel files contain multiple sheets containg related tables labeled, 'capture', 'adult', 'duckling' and 'trap'; in some years, all data was collected in one sheet and one table. 'Capture' and 'trap' sheets are standardized and combined into a temporary data frame. 'Adult' and 'duckling' sheets are standardized and combined into a temporary data frame; with the addition of a new column 'AGE_CLASS'. The code then links the bird-level records with the event-level records using 'CAPTURE #' as the joining variable. Only columns that do not already exist in the bird-level data are added from the event-level table. This allows all adult female and duckling records associated with a capture event to inherit common information such as capture location or trap details while maintaining separate rows for each individual bird. A 'DATA_SOURCE' column is added to indicate that these records originated from brood capture files. 
After brood files have been processed, any empty or invalid results are removed and records with missing 'CAPTURE #' are removed (because they are unable to be linked with a capture event and therefore validate the bird-level data) (*this could be revisited with a different logic, could they be linked by data and band code?*) The code then standardizes column names from the brood capture data frame and the previously created markdata data frame. The files are then combined and a new column 'DATA_SOURCE' is assigned. The resulting table contains all markdata and brood capture data available from years 2006-2015. 
...
```
<details><summary>combined_header_data_total_reduced TABLE *(Click to expand)*</summary>
```{r}
knitr:: kable(combined_header_data_total_reduced)
```
</details>
<details><summary>associated_spp_header_combined TABLE *(Click to expand)*</summary>
```{r}
knitr:: kable(associated_spp_header_combined)
```
</details>
...
After the tables were combined, some editing to the format of the table was done. For some years, the data tables used for collection were used for other projects, because of this, there are some columns in which no data was collected; these columns, containing only NAs are removed from the table. There were two columns that collected the same data with slight variation in name, labeled "H2O_DIST" and "H20_DIST"; these columns were combined. In the column labeled "SITE" there were many variations of data types that mean the same thing; these labels were gone through by hand, determined the category they represent and these entries were altered to match. In the Header table there are many columns that describe other species that have an association with the spectacled eider nest; however, this data was not collected in most years. This results in an overcrowded table with mostly NA values. In an effort to make the Header table easier to work with, a separate table was created for the associated species, related to the Header table by HEADER_ID and NEST_NO. This preserves the data that was collected for associated plant and animal species. There are now two data frames: associated_spp_header_combined (includes identification columns and associated species columns) and combined_header_data_total_reduced (includes header data from 1994-2015, without the columns for associated species). 
All of the coordinates are currently expressed in UTM format using Eastings and Northings. To make these coordinates more useful for plotting, they are converted into the WGS84 system using Latitude and Longitude. This process begins by converting any value in the 'EASTING' and 'NORTHING' columns that is 'NA' to 0, as the sf package required there to be no NAs when converting; these will be converted back into NAs after the conversion process. Then a function from the sf package converts the columns 'EASTING' and 'NORTHING' into a spatial object, specifying the coordinate reference system to UTM zone 33, which uses the EPSG code 32603. The coordinate reference system is then transformed from UTM to WGS84 using the EPSG code 4326. The transformed coordinated are then extracted and placed into new columns, 'LAT' and 'LON'. The placeholder 0s are then converted back into NAs. After the locations are converted to Latitude and Longitude, a boundary defining Kigigak Island area is created with minimum and maximum acceptable latitude and longitude values. A new column is created labeled TRUE_LOCATION with the purpose of describing whether the Latitude and Longitude columns reflect a location that is possibly accurate. If the location is outside of the Kigigak area boundary, we know certainly that those coordinates are inaccurate (will get a label "no" in column), as all data was collected on the Kigigak Island; if the coordinates are within the boundary of Kigigak (will get the label "yes" in the column), there is still the possibility that the coordinates do not accurately represent the location, but there is no way to ensure coordinate accuracy within the island. A dataframe is created that includes only points inside the Kigigak boundary. This data is used to plot the locations using 'leaflet' package. The 'leaflet' map created is then exported to a viewable .html file located in the output folder.
...
The code chunk above is the process of combining the two markdata data frames previously created in sections 6b and 12b. Using the 'bind_rows' function, the two data frames are combined after first standardizing the format of the 'LONGITUDE' columns. After combining, formatting adjustments were made to make the table more usable. There was a mistake in the data entry in the years 2000 and some of 2002, where the year was entered as 1900 and 1902; these dates were transformed to the correct year. A column that includes the complete band number is then created. This column is then combined with the 'DATE' and 'NEST_NO' columns to create a new, unique identifying column, 'MARK_ID'. However, many duplicates remain in this column, as there are oddities and inconsistencies with certain data entires, as well as NA values for either 'BANDNUMBER_COMPLETE', 'DATE', or 'NEST_NO'. Including three identifiers reduces the amount of duplicates by increasing specificity, however, there are still many and are being investigated through review of the hard copies. For some years, the data tables used for collection were used for other projects, because of this, there are some columns in which no data was collected; these columns, containing only NA values are removed from the table. After reviewing the table, there were also two columns that contain 0s in addition to NA values, where the 0s clearly did not represent data and were deemed equivalent to NA values; these columns were also removed.  There were two columns that collected the same data with slight variation in name, labeled "TARSALCODE" and "TARSAL"; these columns were combined. An additional two columns collected the same data with slight variation in name, labeled "NASALCODE" and "NASAL"; these columns were combined. In the column labeled "AGE" there were many variations of data types that mean the same thing; these labels were gone through by hand, determined the category they represent and these entries were altered to match. To eliminate confusion of column names when file is exported to a .csv file, the column "Date orgininally banded" was renamed to "DATE_ORIGINAL_BAND".  
All of the coordinates are currently expressed in UTM format using Eastings and Northings. To make these coordinates more useful for plotting, they are converted into the WGS84 system using Latitude and Longitude. This process begins by converting any value in the 'EASTING' and 'NORTHING' columns that is 'NA' to 0, as the sf package required there to be no NAs when converting; these will be converted back into NAs after the conversion process. Then a function from the sf package converts the columns 'EASTING' and 'NORTHING' into a spatial object, specifying the coordinate reference system to UTM zone 33, which uses the EPSG code 32603. The coordinate reference system is then transformed from UTM to WGS84 using the EPSG code 4326. The transformed coordinated are then extracted and placed into new columns, 'LATITUDE' and 'LONGITUDE'. The placeholder 0s are then converted back into NAs. After the locations are converted to Latitude and Longitude, a boundary defining Kigigak Island area is created with minimum and maximum acceptable latitude and longitude values. A new column is created labeled TRUE_LOCATION with the purpose of describing whether the Latitude and Longitude columns reflect a location that is possibly accurate. If the location is outside of the Kigigak area boundary, we know certainly that those coordinates are inaccurate (will get a label "no" in column), as all data was collected on the Kigigak Island; if the coordinates are within the boundary of Kigigak (will get the label "yes" in the column), there is still the possibility that the coordinates do not accurately represent the location, but there is no way to ensure coordinate accuracy within the island. A dataframe is created that includes only points inside the Kigigak boundary. This data is used to plot the locations using 'leaflet' package. The 'leaflet' map created is then exported to a viewable .html file located in the output folder.
...
The code chunk above is the process of combining the two resight data frames previously created in sections 6c and 12c. The 'NORTHING' column data type is standardized. The two data frames are then combined. After combining, formatting adjustments were made to make the table more usable. There was a mistake in the data entry in the years 2000 and some of 2002, where the year was entered as 1900 and 1902; these dates were transformed to the correct year. Columns "TARSUS", "TARSAL", and "TARSALCODE" collect the same data, these columns were combined into a single column labeled "TARSALCODE". For some years, the data tables used for collection were used for other projects, because of this, there are some columns in which no data was collected; these columns, containing only NAs are removed from the table. There were two columns that collected the same data with slight variation in name, labeled "COMMENTS" and "Comments"; these columns were combined. In the column labeled "FIRST_MARK" there were many variations of data types that mean the same thing; these labels were gone through by hand, determined the category they represent and these entries were altered to match. In the column labeled "RESIGHT_METHOD" there were many variations of data types that mean the same thing; these labels were gone through by hand, determined the category they represent and these entries were altered to match. One of the entries in the "RESIGHT_METHOD" described both an existing data category, and included additional notes; the additional notes were added to the "COMMENTS" column. 
All of the coordinates are currently expressed in UTM format using Eastings and Northings. To make these coordinates more useful for plotting, they are converted into the WGS84 system using Latitude and Longitude. This process begins by converting any value in the 'EASTING' and 'NORTHING' columns that is 'NA' to 0, as the sf package required there to be no NAs when converting; these will be converted back into NAs after the conversion process. Then a function from the sf package converts the columns 'EASTING' and 'NORTHING' into a spatial object, specifying the coordinate reference system to UTM zone 33, which uses the EPSG code 32603. The coordinate reference system is then transformed from UTM to WGS84 using the EPSG code 4326. The transformed coordinated are then extracted and placed into new columns, 'LAT' and 'LON'. The placeholder 0s are then converted back into NAs. After the locations are converted to Latitude and Longitude, a boundary defining Kigigak Island area is created with minimum and maximum acceptable latitude and longitude values. A new column is created labeled TRUE_LOCATION with the purpose of describing whether the Latitude and Longitude columns reflect a location that is possibly accurate. If the location is outside of the Kigigak area boundary, we know certainly that those coordinates are inaccurate (will get a label "no" in column), as all data was collected on the Kigigak Island; if the coordinates are within the boundary of Kigigak (will get the label "yes" in the column), there is still the possibility that the coordinates do not accurately represent the location, but there is no way to ensure coordinate accuracy within the island. A dataframe is created that includes only points inside the Kigigak boundary. This data is used to plot the locations using 'leaflet' package. The 'leaflet' map created is then exported to a viewable .html file located in the output folder.
...
The code chunk above is the process of combining the two visit data frames previously created in sections 6d and 12d. Several columns data types needed to be standardized including, 'PHOTO', 'TIME', and 'CANDLE2'. The data frames are then combined. After combining, formatting adjustments were made to make the table more usable. There was a mistake in the data entry in the years 2000 and some of 2002, where the year was entered as 1900 and 1902; these dates were transformed to the correct year. A unique identifying column is then created by combining 'DATE' and 'NEST_NO' to create, 'VISIT_ID'. For some years, the data tables used for collection were used for other projects, because of this, there are some columns in which no data was collected; these columns, containing only NAs are removed from the table. There were two columns that collected the same data with slight variation in name, labeled "HEN_STAT" and "HEN_STATUS"; these columns were combined. An additional two columns collected the same data with slight variation in name, labeled "EGGS_INVI" and "EGGS_INVIABLE"; these columns were combined. In the column labeled "STATUS" there were many variations of data types that mean the same thing; these labels were gone through by hand, determined the category they represent and these entries were altered to match.
All of the coordinates are currently expressed in UTM format using Eastings and Northings. To make these coordinates more useful for plotting, they are converted into the WGS84 system using Latitude and Longitude. This process begins by converting any value in the 'EASTING' and 'NORTHING' columns that is 'NA' to 0, as the sf package required there to be no NAs when converting; these will be converted back into NAs after the conversion process. Then a function from the sf package converts the columns 'EASTING' and 'NORTHING' into a spatial object, specifying the coordinate reference system to UTM zone 33, which uses the EPSG code 32603. The coordinate reference system is then transformed from UTM to WGS84 using the EPSG code 4326. The transformed coordinated are then extracted and placed into new columns, 'LAT' and 'LON'. The placeholder 0s are then converted back into NAs. After the locations are converted to Latitude and Longitude, a boundary defining Kigigak Island area is created with minimum and maximum acceptable latitude and longitude values. A new column is created labeled TRUE_LOCATION with the purpose of describing whether the Latitude and Longitude columns reflect a location that is possibly accurate. If the location is outside of the Kigigak area boundary, we know certainly that those coordinates are inaccurate (will get a label "no" in column), as all data was collected on the Kigigak Island; if the coordinates are within the boundary of Kigigak (will get the label "yes" in the column), there is still the possibility that the coordinates do not accurately represent the location, but there is no way to ensure coordinate accuracy within the island. A dataframe is created that includes only points inside the Kigigak boundary. This data is used to plot the locations using 'leaflet' package. The 'leaflet' map created is then exported to a viewable .html file located in the output folder.
...
This code section includes post-combination manipulation. These code chunks are located here because they rely on information located in other combined tables that need to be fully established before the information can be included in other tables. The primary goal of this section us to include all of the necessary information in all of the table to make each of them "stand-alone" data tables. 
The goal of the first chunk is to add a DATE_FIRST_FOUND column to the header table, as date was not something that was included in the header data. This information is gathered from matching the minimum date value for a given NEST_NO per year from the visit table, and joining this back to the header table. 
The next block is post-combination column consolidation for markdata. It was found that many columns had escaped the column rename mapping functions applied in the first and second scripts; so there were a extra columns that delivered the same data, just under a slightly different column name. These conflicting columns were first checked for any entry conflicts (for example, if two columns supposedly collecting the same data had different entries for the same row) that would complicate consolidation. After all conflicts were confirmed to be resolved, columns were consolidated. These columns include, 'WGT', 'TARSAL CODE', 'REPLACEMENT NASAL' variants, 'DATE ORIGINALLY BANDED' variants, 'SALINITY' variants, 'POND DEPTH' variants, 'POND SHAPE' variants, 'PERCENT EMERGENT COVER' variants, 'VEGETAION TYPE' variants, 'VEGETATION SPECIES' variants, and 'COMMENTS' and 'REMARKS' (uniting in order to not lose data). Then any records containing only NA or 0 values were removed. 
The next code chunk is adding 'STUDYAREA' to the resight table. This is gathered from the header 'STUDYAREA' column and joined by correlating 'NEST_NO' and 'Year. If there is no correlating NEST_NO and Year, the study area is input as "KIGI" if the 'TRUE_LOCATION' column is a "yes" value. 
The next code chunk is adding STUDYAREA to the egg table.This is gathered from the header 'STUDYAREA' column and joined by correlating 'NEST_NO' and 'Year.
...
This final code block is to export all of the combined and finalized data frames to usable .csv files located in the output folder.
